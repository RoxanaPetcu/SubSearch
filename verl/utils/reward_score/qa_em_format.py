...
def extract_solution(solution_str):
    # Only look at model output, not the prompt
...
def check_decomposition_usage(solution_str: str) -> tuple[bool, bool]:
    """
    Check if decomposition (##) was used and if it led to successful retrieval.
    
    Returns:
        (used_decomposition, decomposition_helped)
    """
    # Check if any search query contains ##
    search_pattern = r'<search>(.*?)</search>'
    search_matches = re.findall(search_pattern, solution_str, re.DOTALL)
    
    used_decomposition = any('##' in match for match in search_matches)
    
    if not used_decomposition:
        return False, False
    
    # Check if decomposition led to multiple information blocks with content
    info_blocks = extract_information_blocks(solution_str)
    
    # Decomposition "helped" if we have information and it contains ## separators
    decomposition_helped = any('##' in block for block in info_blocks)
    
    return used_decomposition, decomposition_helped


# def compute_score_em(solution_str, ground_truth, method='strict', structure_format_score=0, final_format_score=0, retrieval_score=0, format_score=0, score=1., decomposition_bonus=0.):
...
#     bonus = decomposition_bonus if (decomposition_helped and retrieval_correct) else 0
            
#     if answer is None:
#         if is_valid_format:
#             if retrieval_correct:
#                 return structure_format_score + retrieval_score + bonus # 0.3 + bonus
#             else:
...
#             if is_valid_format:
#                 return score + bonus # 1 + bonus (only if decomposition helped get the right answer)
#             else:
